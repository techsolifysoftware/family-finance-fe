...
import { Building, FileText, Save, User, X } from "lucide-react";

interface MemberFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (e: React.FormEvent) => void;
  formData: { name: string; branchId: string; description: string };
        ...
        <div className="bg-primary/5 px-6 py-6 border-b border-primary/10">
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground mt-1.5 ml-12">
              Điền thông tin chi tiết của thành viên mới vào hệ thống quản lý
              dòng họ.
            </DialogDescription>
          </DialogHeader>
        </div>
        ...
        <form onSubmit={onSubmit} className="p-6 space-y-6">
          <div className="space-y-4">
            <div className="space-y-2.5">
              ...
              >
                <User className="w-3.5 h-3.5 text-primary/60" /> Họ và tên hội
                viên
              </Label>
              <Input
                id="name"
            ...
            <div className="space-y-2.5">
              <Label
                ...
                className="text-sm font-bold flex items-center gap-2"
              >
                <Building className="w-3.5 h-3.5 text-primary/60" /> Chi nhánh /
                Hội trực thuộc
              </Label>
              <Select
            ...
            <div className="space-y-2.5">
              <Label
                ...
                className="text-sm font-bold flex items-center gap-2"
              >
                <FileText className="w-3.5 h-3.5 text-primary/60" /> Mô tả / Ghi
                chú
              </Label>
              <Textarea
                id="description"
                placeholder="Thông tin bổ sung về thành viên..."
                className="bg-muted/30 border-border/60 focus:bg-background transition-all rounded-xl pl-4 min-h-[100px] py-3 resize-none"
